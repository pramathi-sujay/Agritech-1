...
if __name__=="__main__":
    asyncio.run(KafkaEventConsumer().process_event())
    
    async def consume(self):
        async for msg in self.consumer:
            event_data = msg.value
            event = Event(**event_data)
            print(f"Received event from sensor {event.sensor_id} with temperature {event.temperature}")
            self.irrigation_service.create_irrigation_data(event)